...
CORS(app, resources={
    r"/*": {
        "origins": "*",  # 开发环境允许所有来源，生产环境请指定具体域名
...
# API 路由
DIST_FOLDER = r'C:\Users\CQ\Desktop\flower_vue\templates\dist'
# 静态文件扩展名列表
...
def predict_single():
    print("收到单图预测请求")  # 日志
    if not predictor:
        return jsonify({"status": "error", "error": "预测器未初始化"})

    if 'image' not in request.files:
        return jsonify({"status": "error", "error": "未上传图片"})

    file = request.files['image']
    if file.filename == '' or not allowed_file(file.filename):
        return jsonify({"status": "error", "error": "无效的图片文件"})
...
def predict_batch():
    """批量预测接口（支持多文件上传）- 优化后直接返回JSON格式结果"""
    print("收到多图预测请求")  # 日志
    try:
        # 检查是否有文件上传
        ...
        return jsonify({
            ...
            'statistics': stats,
            'predictions': formatted_results  # 优化后的结果格式
        }), 200
...
def serve_static_files(path):
    """专门处理静态文件，避免通配符路由处理静态资源"""
    return send_from_directory(os.path.join(DIST_FOLDER, 'static'), path)
...
def serve_routes(path):
    ...
    """
    # 拦截API请求，直接返回404
    api_paths = ['predict/single', 'predict/batch', 'health', 'info']
    if any(path.startswith(api_path) for api_path in api_paths):
        return jsonify({'error': '接口不存在'}), 404

    # 处理静态文件
    static_ext = {'.js', '.css', '.png', '.jpg', '.jpeg', '.gif', '.svg', '.woff', '.woff2', '.ttf', '.eot'}
    file_ext = os.path.splitext(path)[1].lower()
    file_path = os.path.join(DIST_FOLDER, path)

    if file_ext in static_ext and os.path.exists(file_path) and os.path.isfile(file_path):
...
def not_found(error):
    # 区分API请求和页面请求：如果是API请求返回JSON错误，否则返回前端页面
    accept_header = request.headers.get('Accept', '')
    if 'application/json' in accept_header or request.path.startswith('/predict/') or request.path in ['/health',
                                                                                                       '/info']:
        return jsonify({'error': '接口不存在'}), 404
    # 页面请求返回前端入口文件
...
if __name__ == '__main__':
    ...
    app.run(
        ...
        threaded=True,  # 启用多线程
        use_reloader=False  # 禁用自动重载，避免启动两次
    )
